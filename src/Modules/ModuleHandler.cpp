...
namespace Crash::Modules
{
	namespace detail
	{
		...
		private:
			[[nodiscard]] static std::span<const std::byte> get_image(::HMODULE a_module)
			{
				...
				const auto ntHeader = REX::ADJUST_POINTER<::IMAGE_NT_HEADERS64>(dosHeader, dosHeader->e_lfanew);
		...
		_path(std::move(a_path))
	{
		...
		auto ntHeader = REX::ADJUST_POINTER<::IMAGE_NT_HEADERS64>(dosHeader, dosHeader->e_lfanew);
		std::span sections(
