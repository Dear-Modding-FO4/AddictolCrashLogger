...
namespace Crash::Introspection::Heap
{
	namespace
	{
		[[nodiscard]] bool is_valid_heap_handle(HANDLE a_heap) noexcept
		{
			...
			try
			{
				...
				ULONG heap_info = 0;
				if (::HeapQueryInformation(a_heap, HeapCompatibilityInformation, &heap_info, sizeof(heap_info), nullptr))
					return true;
			}
			catch (...)
			{
	...
	std::optional<HeapInfo> analyze_heap_pointer(const void* a_ptr)
	{
		...
		// Check configuration
		if (!Settings::bHeapAnalysis.GetValue())
			return std::nullopt;

		const auto max_heaps = Settings::iMaxHeapsToCheck.GetValue();
		const auto max_iterations = Settings::iMaxHeapIterationsPerHeap.GetValue();
