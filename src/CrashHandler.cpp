...
namespace Crash
{
	...
	void Callstack::print_raw_callstack(spdlog::logger& a_log) const
	{
		...
		{
			a_log.critical(fmt::format(fmt::runtime(format), i, reinterpret_cast<std::uintptr_t>(_frames[i].address())));
		}
	}
	...
	std::filesystem::path GetF4SELogDirectory()
	{
		...
		{
			if (auto fileSink = std::dynamic_pointer_cast<spdlog::sinks::basic_file_sink_mt>(sink))
			{
				return std::filesystem::path(fileSink->filename()).parent_path();
			}
		}
